...
const Button = ({ onClick, children, isLoading, className }: ButtonProps) => {
  const classButton = useMemo(() => {
    const classLoading = isLoading ? 'disabled cursor-not-allowed disabled:bg-blue-400' : ''
  ...
  return (
    <button type="button" onClick={onClick} disabled={isLoading} className={classButton}>
      {children}
    </button>
  )
}
